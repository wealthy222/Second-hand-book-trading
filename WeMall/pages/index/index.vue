...
export default {
	...
	//	 this.productList = []; // 清空列表
      //  setTimeout(function () {
		//	that.dataGet(this.bookNameRec,this.bookCartRec,this.bookOrderRec).then(() => {
  ...
  //});
	...
	methods: {
		...
		async dataGet(bookNameRec,bookCartRec,bookOrderRec){
			const res = await this.$JerryRequest({
				url:"/getRecomend?bookName="+bookNameRec+"&bookCart="+bookCartRec+"&bookOrder="+bookOrderRec,
			})
