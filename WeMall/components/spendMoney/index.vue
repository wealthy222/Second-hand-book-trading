			...
			passwordBox(num) {
				...
				if (this.passwordArray.length == 6) {
					...
					if(this.num>0 && this.price>0){
						this.$api.insertUserOrder(uni.getStorageSync("bookId"),this.num,this.price,uni.getStorageSync("id"),this.sellerId,uni.getStorageSync("username"),uni.getStorageSync("mobile"),this.address);
						this.$api.updateBooknumber(uni.getStorageSync("bookId"),this.num);
